...
```
@make target="test" args=["PATTERN=issues"]
```

PATTERN supports shorthand - `issues` expands to `test/**/*issues*_spec.lua`. Full paths also work:

```
@make target="test" args=["PATTERN=test/issues_spec.lua"]
```
...
- `test/install_deps.lua` - downloads luaunit test framework
...
- `test/helpers.lua` - shared mock utilities for `github.util`
...
│   ├── workflows/
│   │   ├── test.yml           # CI tests
│   │   ├── luarocks.yml       # LuaRocks publish
│   │   └── release-please.yml # Automated release
